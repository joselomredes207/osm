...
window.togglePinnedPublicacion=async(id,current)=>{try{await initFirebase();await fb.updateDoc(fb.doc(db,'noticias',id),{pinned:!current});await refreshVisibleAfterInteraction()}catch(e){console.error(e);alert('No se pudo fijar/desfijar la publicación')}};
window.editarAutorPublicacion=async(id,currentAuthor={})=>{await initFirebase();let modal=document.getElementById('author-editor-modal');if(!modal){modal=document.createElement('div');modal.id='author-editor-modal';modal.className='title-editor-modal';modal.innerHTML=`<div class="title-editor-box"><button class="title-editor-close" type="button">&times;</button><h3>Editar autor visible</h3><p>Cambia quién aparece como autor de la publicación.</p><select id="author-editor-select" class="title-editor-input"></select><div id="author-editor-custom" class="custom-author-box" style="display:none"><input id="author-editor-name" class="input" placeholder="Nombre del autor"><input id="author-editor-club" class="input" placeholder="Club / cargo / descripción"></div><div class="title-editor-actions"><button class="title-editor-cancel" type="button">Cancelar</button><button class="title-editor-save" type="button">Guardar autor</button></div><div class="title-editor-status" id="author-editor-status"></div></div>`;document.body.appendChild(modal);modal.querySelector('.title-editor-close').onclick=()=>modal.classList.remove('open');modal.querySelector('.title-editor-cancel').onclick=()=>modal.classList.remove('open')}const sel=modal.querySelector('#author-editor-select');sel.innerHTML='<option value="profile-current">Mi perfil actual</option>'+Object.entries(UPLOAD_AUTHORS).map(([k,a])=>`<option value="${k}">${a.name} · ${a.club}</option>`).join('')+'<option value="custom">➕ Crear otro autor</option>';sel.value=currentAuthor.selectedAuthorId||'profile-current';const custom=modal.querySelector('#author-editor-custom');const st=modal.querySelector('#author-editor-status');st.textContent='';sel.onchange=()=>custom.style.display=sel.value==='custom'?'block':'none';sel.onchange();modal.querySelector('.title-editor-save').onclick=async()=>{try{const owner=currentProfile();let author;if(sel.value==='profile-current'){author={name:owner.name,club:owner.club,avatar:owner.avatar,photo:owner.photo,role:'Entrenador',selectedAuthorId:'profile-current'}}else if(sel.value==='custom'){const n=(modal.querySelector('#author-editor-name').value||'').trim();const c=(modal.querySelector('#author-editor-club').value||'').trim();if(!n){st.textContent='Escribe el nombre del autor.';return}author={name:n,club:c||'Autor personalizado',avatar:owner.avatar,photo:owner.photo,role:'Autor personalizado',selectedAuthorId:'custom'}}else{author={...UPLOAD_AUTHORS[sel.value],selectedAuthorId:sel.value,selectedAuthorRole:UPLOAD_AUTHORS[sel.value].role||''}}await fb.updateDoc(fb.doc(db,'noticias',id),{author});st.textContent='✅ Autor guardado';await refreshVisibleAfterInteraction();setTimeout(()=>modal.classList.remove('open'),450)}catch(e){console.error(e);st.textContent='❌ No se pudo guardar'}};modal.classList.add('open')};
window.filtrarGaleria=function(cat){const g=cats[cat]?.g;if(!g)return;const q=(document.getElementById('buscar-'+cat)?.value||'').toLowerCase().trim();const role=document.getElementById('filtro-'+cat)?.value||'';const order=document.getElementById('orden-'+cat)?.value||'recientes';const cards=[...g.querySelectorAll('.imgcard')];cards.forEach(card=>{const hay=(card.dataset.title+' '+card.dataset.author).includes(q);const okRole=!role||(card.dataset.role||'').includes(role);card.style.display=(hay&&okRole)?'':'none'});cards.sort((a,b)=>{if((b.dataset.pinned||'0')!==(a.dataset.pinned||'0'))return Number(b.dataset.pinned)-Number(a.dataset.pinned);if(order==='likes')return Number(b.dataset.likes||0)-Number(a.dataset.likes||0);if(order==='comentarios')return Number(b.dataset.comments||0)-Number(a.dataset.comments||0);if(order==='autor')return (a.dataset.author||'').localeCompare(b.dataset.author||'');return Number(b.dataset.date||0)-Number(a.dataset.date||0)}).forEach(c=>g.appendChild(c))};
function applyGalleryFilters(cat){try{window.filtrarGaleria(cat)}catch(_){}}
function commentTimeMs(c){const f=c&&c.fecha;if(!f)return 0;if(typeof f.toMillis==='function')return f.toMillis();if(typeof f.seconds==='number')return f.seconds*1000+Math.floor((f.nanoseconds||0)/1000000);const d=new Date(f);return isNaN(d.getTime())?0:d.getTime()}
function ensureReplyDigestModal(){let modal=document.getElementById('reply-digest-modal');if(modal)return modal;modal=document.createElement('div');modal.id='reply-digest-modal';modal.className='reply-digest-modal';modal.innerHTML=`<div class="reply-digest-box"><button type="button" class="reply-digest-close" id="reply-digest-close">&times;</button><h3>💬 Actividad nueva</h3><p>Respuestas o comentarios nuevos desde tu última visita.</p><div class="reply-digest-list" id="reply-digest-list"></div><div class="reply-digest-actions"><button type="button" class="btn" id="reply-digest-ok">Entendido</button></div></div>`;document.body.appendChild(modal);const close=()=>modal.classList.remove('open');modal.querySelector('#reply-digest-close').onclick=close;modal.querySelector('#reply-digest-ok').onclick=close;modal.addEventListener('click',e=>{if(e.target===modal)close()});return modal}
async function checkReplyDigest(){try{if(!profileReady())return;await initFirebase();const me=currentProfile().id;const last=Number(localStorage.getItem('premier_reply_seen_at')||'0');const snap=await fb.getDocs(noticias);const events=[];snap.forEach(d=>{const post=d.data()||{},comments=Array.isArray(post.comments)?post.comments:[],byId={};comments.forEach(c=>{if(c&&c.id)byId[c.id]=c});comments.forEach(c=>{const t=commentTimeMs(c);if(!c||c.profileId===me||t<=last)return;const parent=c.parentId?byId[c.parentId]:null;if(parent&&parent.profileId===me){events.push({type:'reply',postId:d.id,post,comment:c,time:t});return}const postOwnerId=post.ownerId||(post.owner&&post.owner.id)||(post.author&&post.author.id);if(!c.parentId&&postOwnerId===me){events.push({type:'post-comment',postId:d.id,post,comment:c,time:t})}})});events.sort((a,b)=>b.time-a.time);localStorage.setItem('premier_reply_seen_at',String(Date.now()));paintRecentActivity(events);if(!events.length)return;const modal=ensureReplyDigestModal(),list=modal.querySelector('#reply-digest-list');list.innerHTML='';events.slice(0,12).forEach(r=>{const item=document.createElement('button');item.type='button';item.className='reply-digest-item';const title=cleanPostTitle(r.post.titulo)||'Publicación sin título';const msg=r.type==='reply'?`${r.comment.name||'Alguien'} te respondió`:`${r.comment.name||'Alguien'} comentó en una publicación tuya`;item.innerHTML=`<b>${msg}</b><span>${title}</span><p></p>`;item.querySelector('p').textContent=r.comment.text||'';item.onclick=()=>{modal.classList.remove('open');openPostLightbox(r.post.src,r.postId,r.post)};list.appendChild(item)});modal.classList.add('open')}catch(e){console.warn('Resumen de respuestas no disponible',e)}}
function paintRecentActivity(events=[]){const host=document.getElementById('recent-activity-list');if(!host)return;if(!events.length){host.innerHTML='<div class="activity-empty">Sin respuestas nuevas desde tu última visita.</div>';return}host.innerHTML='';events.slice(0,6).forEach(ev=>{const b=document.createElement('button');b.type='button';b.className='activity-item';const title=cleanPostTitle(ev.post.titulo)||'Publicación sin título';b.innerHTML=`<b>${ev.comment.name||'Alguien'}</b><span>${ev.type==='reply'?'te respondió en':'comentó en'} ${title}</span>`;b.onclick=()=>openPostLightbox(ev.post.src,ev.postId,ev.post);host.appendChild(b)})}
window.checkReplyDigest=checkReplyDigest;window.scrollConsejoAlbum=function(dir){const el=document.getElementById('consejo-track');if(el)el.scrollBy({left:dir*320,behavior:'smooth'})};setTimeout(checkReplyDigest,1200);
